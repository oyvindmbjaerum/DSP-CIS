...
function   [berTransmission, ratio, error_locations] = main(frame_length, M, L, snr, channel_constants)
    ...
    mask = ones(frame_length, 1); %Comment out this line if you want to use ON-OFF bitloading, goes from ber 0,2 to 0,3 is that right?
    ...
    % OFDM demodulation
    rxQamStream = ofdm_demod(rxOfdmStream, frame_length, L, channel_constants, mask, mask); %(1:end - (length(channel_constants) -1),:)
